...
import json
import re
def gethandphone(s):       #获取电话号码
    res = re.search(r'\d{11}', s)
    if res == None:
        return ""
    return res.group(0)

def getprovince(s):      #获取直辖市/省/自治区/特别行政区
    res = re.search(("(.*?省)|(.*?自治区)|(.*?特别行政区)|上海市|北京市|天津市|重庆市"), s)
    if res == None:
        if s[0:3] == "黑龙江":
            return "黑龙江"
        if s[0:3] == "内蒙古":
...
while 1:#循环直到读到END
    str1=input();#读入input到inputraw
    if(str1=="END"):#如果inputraw等于END退出while的循环
        break
    imformation = str1
    s = imformation[0]
    imformation = imformation[2:]
    document = imformation.split(",")
    NAME = document[0]
    REMAIN = document[1]
    HANDPHONE = gethandphone(REMAIN)
    REMAIN = REMAIN.replace(HANDPHONE,"",1)#过滤电话号码
    REMAIN = REMAIN.replace(".","",1)

    PROVINCE = getprovince(REMAIN)
    if PROVINCE in ('北京', '上海', '天津', '重庆'):
        PROVINCE = PROVINCE
        REMAIN = REMAIN.replace(PROVINCE, PROVINCE+'市',1)
    elif PROVINCE in ('北京市', '上海市', '天津市', '重庆市'):
        PROVINCE = PROVINCE[0:2]
    elif PROVINCE == "广西":
        REMAIN = REMAIN.replace(PROVINCE, "",1)
    ...
    CITY = getcity(REMAIN)
    if CITY != "":
        REMAIN = REMAIN.replace(CITY, "",1)
        if CITY[-1] != "市" and CITY[-3:-1] != "自治":
            CITY = CITY+"市" 

    COUNTRY = getcountry(REMAIN)
    if COUNTRY != "":
    #L = len(COUNTRY)
    #if COUNTRY[-1] == REMAIN[L-1]:
         REMAIN = REMAIN.replace(COUNTRY, "",1)
    #else:
        #REMAIN = REMAIN.replace(COUNTRY[:-1], "",1)

    TOWN = gettown(REMAIN)
    REMAIN = REMAIN.replace(TOWN, "",1)
    DETAIADDR = REMAIN

    ROAD = getroad(REMAIN)
    REMAIN = REMAIN.replace(ROAD, "",1)

    DOORNUM = getdoornum(REMAIN)
    REMAIN = REMAIN.replace(DOORNUM, "",1)

    DICT1 = {}

    ADDRLIST1 = []
    if s == '1':
        ADDRLIST1.append(PROVINCE)
        ADDRLIST1.append(CITY)
        ADDRLIST1.append(COUNTRY)
        ADDRLIST1.append(TOWN)
        ADDRLIST1.append(DETAIADDR)
    else:
        ADDRLIST1.append(PROVINCE)
        ADDRLIST1.append(CITY)
        ADDRLIST1.append(COUNTRY)
        ADDRLIST1.append(TOWN)
        ADDRLIST1.append(ROAD)
        ADDRLIST1.append(DOORNUM)
        ADDRLIST1.append(REMAIN)
    DICT1["姓名"] = NAME
    DICT1["手机"] = HANDPHONE
    DICT1["地址"] = ADDRLIST1
